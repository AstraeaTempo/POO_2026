...
Instrucciones: Crea una función donde definas 
let alumnos = ["Juan", "María", "Pedro"];.
Un nuevo alumno, "Luis", llega primero a la fila(usa.unshift()).
El primer alumno de la fila entra a clases
(usa.shift() y guarda ese nombre en una variable).
Muestra un alert() que diga: "Entró [nombre] y la fila ahora es: 
[el resto del arreglo]".*/

function gestionaFila() {
    let alumnos = ["Juan", "María", "Pedro"];
...
Instrucciones: Crea una función que reciba el arreglo 
...
Suma ambos valores y muestra el resultado con 
un alert() que explique qué posiciones sumaste 
...
function traduccionAcciones() {
    ...
    let verbos = [["codifica", "analiza"], ["falla", "reiniciar"]];
...
Instrucciones: Crea una función con un arreglo 
let pacientes = ["Ana", "Betto"];.
Llegan dos pacientes más: "Carlos" y "Diana" (usa .push()).
Atienden al primero (usa .shift()).
Llega una urgencia: "Sonia" debe ser 
atendida de inmediato (usa .unshift()).
Muestra cuántos pacientes quedan y 
